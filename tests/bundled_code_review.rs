//! The bundled code-review plugin renders the native unified diff stream.
//!
//! The fourth Phase 4 port and the first that is **partial by design**: the
//! native view (`src/ui/code_review.rs`) is the largest pane in thurbox, and what
//! is reproduced here is its core — the unified diff stream's lines, with their
//! line-number gutter, their syntax-coloured bodies, their insertion/deletion row
//! tints and the cursor's row. Everything else is out of scope, itemised in the
//! change's proposal and pinned below by
//! [`the_out_of_scope_surface_is_absent_rather_than_approximated`].
//!
...
//!
//! 1. here: the plugin's tree equals `ui::code_review::diff_stream_tree`;
//! 2. in `ui::code_review`'s own tests: that builder and the untouched
//!    `unified_diff_line` **paint the same row**.
//!
...
use thurbox::session::pane_context::{PaneContext, ReviewLineSnapshot, ReviewSnapshot};
use thurbox::session::review::{DiffLine, DiffLineKind};
use thurbox::session::view_tree::{ViewNode, MAX_NODES};
use thurbox::ui::code_review::diff_stream_tree;
...
/// One comparison case. Both sides are derived from the same lines, so an
/// equality failure is a statement about the plugin rather than about two
/// hand-written fixtures drifting apart.
struct Case {
    name: &'static str,
    lines: Vec<(String, DiffLine)>,
    cursor: Option<usize>,
    number_width: usize,
}
...
fn context(old: u32, new: u32, text: &str) -> DiffLine {
    line(DiffLineKind::Context, Some(old), Some(new), text)
}

fn added(new: u32, text: &str) -> DiffLine {
    line(DiffLineKind::Add, None, Some(new), text)
}

fn removed(old: u32, text: &str) -> DiffLine {
    line(DiffLineKind::Del, Some(old), None, text)
}

/// The rows of one file, as the stream lists them.
fn in_file(path: &str, lines: Vec<DiffLine>) -> Vec<(String, DiffLine)> {
    lines.into_iter().map(|l| (path.to_string(), l)).collect()
}

impl Case {
    fn native_tree(&self) -> ViewNode {
        diff_stream_tree(&self.lines, self.cursor, self.number_width)
    }

    /// The same lines as the snapshot a plugin reads.
    ///
    /// Mirrors `App::build_review_snapshot`: the kernel publishes a path, the two
    /// line numbers where each exists, the kind, the raw text, the cursor and the
    /// gutter width — and no colouring, no window and no padding.
...
/// part of a diff line the plugin gets wrong rather than only that it does.
fn cases() -> Vec<Case> {
    let case = |name, lines, cursor, number_width| Case {
        name,
        lines,
        cursor,
        number_width,
    ...
    vec![
        ...
        // The three kinds together, which is what a hunk is.
        case(
        ...
        case(
            ...
            vec![
                (
                    "conf/app.toml".to_string(),
                    context(1, 1, "# a hash comment"),
                ),
                ("db/schema.sql".to_string(), context(1, 1, "-- a dash one")),
                ("src/main.rs".to_string(), context(1, 1, "#[derive(Debug)]")),
                ("run.sh".to_string(), added(2, "# shell")),
            ],
            None,
            3,
        ),
        ...
        case(
            ...
            in_file("src/a.rs", vec![context(1, 1, "a"), added(2, "b")]),
            Some(1),
            2,
        ),
        case(
            "the cursor on a deletion",
            in_file("src/a.rs", vec![removed(1, "a"), context(1, 2, "b")]),
            Some(0),
            2,
        ),
        ...
        case(
            ...
            in_file("src/a.rs", vec![context(1, 1, "a"), context(2, 2, "b")]),
            None,
            2,
        ),
    ]
...
/// diff stream's rows, reproduced from two declared capabilities.
#[test]
...
fn the_plugin_paints_the_native_frame_when_the_pane_scrolls() {
    ...
    let lines: Vec<(String, DiffLine)> = (0..20)
        .map(|i| {
            (
                "src/a.rs".to_string(),
                if i % 3 == 0 {
...
fn the_hosts_bounds_are_reached_by_an_ordinary_diff() {
    ...
    let dense = |n: usize| -> Vec<(String, DiffLine)> {
        (0..n)
            .map(|i| {
                (
                    "src/a.rs".to_string(),
                    context(i as u32 + 1, i as u32 + 1, body),
                )
            })
            .collect()
    };
    ...
    let one = diff_stream_tree(&dense(1), None, 2);
...
/// **The out-of-scope surface, asserted absent.** The native view also draws file
/// headers, hunk headers, comments with classification badges, a review summary,
/// reviewed marks, a find bar, a target picker and a footer. None of it is
/// reproduced, and none of it is *approximated* either — a pane that drew a
/// plausible-looking file header would agree with nothing and make the record a
/// claim about a pane that does not exist.
///
/// So this asserts on the text of every run: the plugin's pane contains a diff and
/// nothing that imitates the rest of the view.
#[test]
fn the_out_of_scope_surface_is_absent_rather_than_approximated() {
    ...
    // The body it *does* draw.
    assert!(text.contains("pub struct Row {"), "{text:?}");
    for absent in [
        "@@",      // a hunk header
        "─",       // a file header's rule
        "▾",       // a file header's fold chevron
        "▸",       // a comment's badge lead, and a folded file's chevron
        "✓",       // a reviewed mark
        "[issue]", // a classification badge
        "Review summary",
        "Search", // the find bar
...
/// Before the host has published anything the reader answers "no diff", so the
