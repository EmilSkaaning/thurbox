//! The kernel's model of the session list: the order thurbox navigates it in, the
//! reorder and sort that mutate that order, the match positions a search resolves,
//! and the width-free resolution of one rendered row.
//!
//! Not a renderer. Every function here is a pure function of `&[&SessionInfo]` plus a
//! cursor, and none of them knows a width, a colour or a widget. That is why the
//! module is here rather than in the pane that draws the list: `compute_session_order`
//! is the single comparator `App`'s `Ctrl+J`/`Ctrl+K` navigation shares with the
//! renderer, and [`move_in_order`] / [`sort_alphabetically_within_groups`] are the
//! primitives behind `Shift+J` / `Shift+K` / `Shift+S`, which renumber
//! `sessions.display_order` densely and persist it. A comparator keyboard navigation
//! depends on is model; holding it in `ui` made the coordinator depend on the layer
//! that is supposed to depend on *it* (ADR-60).
//!
//! **Geometry stays with the pane.** [`resolve_rows`] leaves
//! [`SessionRow::status_text`] unset, because deciding how much of an agent's reported
...
pub const NO_REPO_GROUP: &str = "(no repo)";
...
pub fn repo_set_key(repo_names: &[String]) -> String {
...
pub fn group_display(info: &SessionInfo) -> String {
    repo_set_display(&info.repo_display_names)
}

/// [`group_display`] over bare repo names (see [`repo_set_key`]).
pub fn repo_set_display(repo_names: &[String]) -> String {
    if repo_names.is_empty() {
        return NO_REPO_GROUP.to_string();
    }
...
fn nest_group_members(sessions: &[&SessionInfo], members: &[usize]) -> Vec<(usize, u8)> {
    ...
    let id_to_member: HashMap<super::SessionId, usize> =
...
/// needs no width, no focus and no session set. It mirrored the tasks and automations
/// panes' own resolved rows, both gone with their renderers (ADR-53, ADR-56): this is
/// the last of the three, and unlike the automations pane (ADR-55) it still fits
/// `status_text` itself rather than declaring a yielding run (ADR-52) — one adoption its
/// own handover owes.
...
/// width, and that is `ui::project_list::resolve_items`' job.
pub fn resolve_rows(inputs: &RowInputs<'_>) -> Vec<(Option<String>, SessionRow)> {
    ...
    let visible_ids: std::collections::HashSet<super::SessionId> =
...
/// carries none.
pub fn agent_status_text(info: &SessionInfo) -> Option<String> {
